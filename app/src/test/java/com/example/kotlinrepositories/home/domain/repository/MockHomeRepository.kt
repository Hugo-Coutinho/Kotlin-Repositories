...
object MockHomeRepository {
    ...
    fun getMockKotlinRepositories(): Observable<List<HomeEntity>> {
        return Observable.just(listOf(this.mockEntity()))
    }

    private fun mockEntity(): HomeEntity {
        val owner = Owner("android","https://avatars3.githubusercontent.com/u/32689599?v=4")
        return HomeEntity("architecture-samples",false, owner.login, "A collection of samples to discuss and showcase different architectural tools and patterns for Android apps.","https://github.com/google/flexbox-layout",36642,10175)
    }
}